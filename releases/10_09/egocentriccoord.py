...
def GetEgocentricCoordinatesTargets(mocap, surfaceMocap, avatar, surfaceAvatar, checkLimbDistanceFlag=True):
    ...
    EgocentricCoordinate(rhand)
    EgocentricCoordinate(lhand)
    # EgocentricCoordinate(rforearm)
    # EgocentricCoordinate(lforearm)
    EgocentricCoordinate(rfoot)
    EgocentricCoordinate(lfoot)
    # EgocentricCoordinate(rlowleg)
    # EgocentricCoordinate(llowleg)
    ...
    for frame in range(mocap.frames):
        ...
        vectors = getVectors(mocap, frame)
        jointpositions = getJointsPositions(mocap, frame)
        mesh = getMeshPositions(mocap, surfaceMocap, frame)
        ...
        for joint in [rhand, lhand, rfoot, lfoot]:
            ...
            if joint == rhand or joint == lhand or joint == rfoot or joint == lfoot:
                jointSurfaceNormal = extremityNormal(mocap, joint, frame)
            ...
            #Limbs components
            for values_returned in pathnormCalcLimb(joint, mocap, 'limb', frame, vectors, jointpositions, surfaceMocap):
...
#            dispvector = distance - surfaceMocap.getPoint('foreRight').radius
        ...
        vectors = getVectors(avatar, frame)
        jointpositions = getJointsPositions(avatar, frame)
        mesh = getMeshPositions(avatar, surfaceAvatar, frame)
        lvec_fore, rvec_fore, lvec_arm, rvec_arm, lvec_clavicle, rvec_clavicle, vec_neck, vec_spine, lvec_femur, rvec_femur, lvec_upleg, rvec_upleg, lvec_lowleg, rvec_lowleg = vectors
        ...
        for joint,egoindex in zip([rhand_ava, lhand_ava, rfoot_ava, lfoot_ava],[0,1,2,3]):
            #Get the ego coordinates of the mocap animation joint
            ...
            for values_returned in DenormEgoLimb(joint, avatar, surfaceAvatar, frame, vectors, jointpositions, ego, i+1):
...
#            return ego.egolist, targets#, taulist, vec_displacement
    ...
    return EgocentricCoordinate.egolist#targets, taulist, vec_displacement
